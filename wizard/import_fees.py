...
class ImportFees(models.TransientModel):
    _name = 'import.fees.wizard'
    _description = 'Import Fees from Excel'

    excel_file = fields.Binary(string='Excel File', required=True,
                                help='Upload the fees Excel file')
    filename = fields.Char(string='Filename')
    import_batch = fields.Char(string='Import Batch Name',
                                 default=lambda self: f"FEES_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
                                 required=True,
    ...
    def action_import(self):
        ...
        existing_batch = self.env['ics.fee.staging'].search([
        ...
        # Process rows
        staging_obj = self.env['ics.fee.staging']
        imported_count = 0
        updated_count = 0
        error_count = 0
        log_messages = []
        ...
        for row_idx, row in enumerate(sheet.iter_rows(min_row=self.skip_header_rows + 1), start=self.skip_header_rows + 1):
            try:
                # Extract cell values
                values = {}
                for col_idx, cell in enumerate(row[:30], start=1):  # Max 30 columns
